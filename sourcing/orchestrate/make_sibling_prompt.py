#!/usr/bin/env python3
"""Build the self-contained prompt for one batch session: rules + line numbers + return instructions.
Usage: python3 make_sibling_prompt.py batch.tsv batch_name parent_session_id > prompt.txt"""
import sys
...
1. Build an HTML file `results.html` whose entire content is:
   <title>Quotables results {name}</title>
...
2. Publish it with the Artifact tool: file_path=results.html, favicon "📚", description "Sourcing results for batch {name}". Note the URL it returns.
3. Notify the parent session by calling the `create_trigger` tool of the remote-session MCP server with:
     name: "sourcing results {name}"
